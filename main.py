                ...
                if selected_class_str:
                    ...
                    # === CASE 2: 내부수업 (기존 로직 유지) ===
                        ...
                        if class_attend.empty:
                            ...
                            with m3: st.markdown(style_metric("과목구분 실인원", f"<span style='margin-left: 20px;'>{c_sub}명</span>", "동일인이라도 수강과목 다르면 따로 집계"), unsafe_allow_html=True)
                            with m4: st.markdown(style_metric("과목반기구분 실인원", f"<span style='margin-left: 20px;'>{c_sub_per}명</span>", "수강과목, 기간(반기) 다르면 따로 집계"), unsafe_allow_html=True)
        ...
        # =========================================================================
            ...
            if not df_u.empty and not df_c.empty and not df_a.empty:
                ...
                if not df_a.empty and not df_c.empty and not df_u.empty:
                    ...
                    df_ext['attendance_date'] = pd.to_datetime(df_ext['attendance_date']) # [추가] 날짜 변환
                    ...
                    # 외부 데이터도 반기 컬럼 필요하면 생성 (여기서는 필터링 용도로 사용 안하면 생략 가능하나 통일성 위해 생성)
                    ...
                    with m3: st.markdown(style_metric("③ 과목구분 실인원", f"{c_sub:,}명", "동일인이라도 수강과목 다르면 따로 집계"), unsafe_allow_html=True)
                    with m4: st.markdown(style_metric("④ 과목반기구분 실인원", f"{c_sub_per:,}명", "수강과목, 기간(반기) 다르면 따로 집계"), unsafe_allow_html=True)
